...
class RoleController extends Controller
{
    ...
    function addData(Request $request){
      ...
      try{
        ...
        $dataModel->is_super_admin_role=$request->is_super_admin_role??0; 
        $res=$dataModel->save();
        if(!$res){
            DB::rollBack();
...
function updateData(Request $request){
  ...
  if ($validator->fails())
  return response (["response"=>400],400);
  if($request->id==14||$request->id==16||$request->id==18||$request->id==21){
    return Helpers::successResponse("You can't update the Subper Admin, Clinic,  Doctor, and Front Desk roles.");
  }
  try{
    $timeStamp= date("Y-m-d H:i:s");
    DB::beginTransaction(); 
    ...
    if(isset($request->is_super_admin_role)){ 
      $dataModel->is_super_admin_role=$request->is_super_admin_role??0; 
...
function deleteData(Request $request){
  ...
  return response (["response"=>400],400);
if($request->id==14||$request->id==16||$request->id==18||$request->id==21){
  return Helpers::successResponse("You can't delete the Subper Admin, Clinic,  Doctor, and Front Desk roles.");
}
    try{ 
            $dataModel= RoleModel::where("id",$request->id)->first();
